...
def filter_text_pairs(text_pairs: tuple, max_left_length: int, max_right_length: int):
    ...
    if (max_left_length <= 0) or (max_right_length <= 0):
        ...
        max_left_length = left_lengths[int(round(float(n_pairs - 1) * 0.85))]
        max_right_length = right_lengths[int(round(float(n_pairs - 1) * 0.85))]
    input_texts = []
    target_texts = []
    for idx in range(n_pairs):
...
def main():
    ...
    if (model_name is not None) and os.path.isfile(model_name):
        ...
        vae = Conv1dTextVAE(input_embeddings=en_fasttext_model, output_embeddings=ru_fasttext_model,
                            n_filters=(256, 512), kernel_size=3, latent_dim=500, max_epochs=max_epochs,
                            verbose=verbose, batch_size=minibatch_size, output_onehot_size=20000,
